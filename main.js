...
function buildTOC(entryList) {
    ...
    entryList.forEach((e, i) => {
        let cat = (typeof e.category === "object" ? (e.category[currentLang] || Object.values(e.category)[0]) : e.category);
    ...
    if (Array.isArray(entry.solution)) {
        codes = entry.solution.map(sol =>
            `<div class="code-block">
                ${sol.description ? `<div class="code-description">${typeof sol.description === 'object' ? (sol.description[currentLang] || Object.values(sol.description)[0]) : sol.description}</div>` : ''}
                <pre class="code-pre">
        ...
        codes = `<div class="code-block">
            ${entry.solution.description ? `<div class="code-description">${typeof entry.solution.description === 'object' ? (entry.solution.description[currentLang] || Object.values(entry.solution.description)[0]) : entry.solution.description}</div>` : ''}
            <pre class="code-pre">
                <button class="copy-btn" onclick="copyCode(this)">Copy</button>
...
document.getElementById('searchInput').addEventListener('input', function(e) {
    ...
    filteredEntries = entries.filter(entry =>
        (typeof entry.title === "object" ? (entry.title.de + " " + entry.title.en) : entry.title).toLowerCase().includes(value) ||
        (typeof entry.problem === "object" ? (entry.problem.de + " " + entry.problem.en) : entry.problem).toLowerCase().includes(value) ||
        (typeof entry.category === "object" ? (entry.category.de + " " + entry.category.en) : entry.category).toLowerCase().includes(value) ||
